...
- Constants: `SAMANSEO_VERSION`, `SAMANSEO_PATH`, `SAMANSEO_URL`
- Namespace: `SamanSEO`
...
**Files:** All files containing `SAMANSEO_`
...
**Verification:** `grep -r "SAMANSEO_" includes/` returns no results

---

### Task 3: Update namespace declarations

**Files:** All 73 files with `namespace SamanSEO`

1. Replace all namespace declarations:
   - `namespace SamanSEO;` → `namespace SamanLabs\SEO;`
   - `namespace SamanSEO\Api;` → `namespace SamanLabs\SEO\Api;`
   - `namespace SamanSEO\Service;` → `namespace SamanLabs\SEO\Service;`
   - `namespace SamanSEO\Integration;` → `namespace SamanLabs\SEO\Integration;`
   - `namespace SamanSEO\Updater;` → `namespace SamanLabs\SEO\Updater;`
   - `namespace SamanSEO\Api\Assistants;` → `namespace SamanLabs\SEO\Api\Assistants;`
   - `namespace SamanSEO\Helpers;` → `namespace SamanLabs\SEO\Helpers;`
   - `namespace SamanSEO\Internal_Linking;` → `namespace SamanLabs\SEO\Internal_Linking;`

**Verification:** `grep -r "namespace SamanSEO" includes/` returns no results

---

### Task 4: Update namespace references (use statements and fully qualified names)

**Files:** All files with `SamanSEO\\` or `\SamanSEO\`

1. Replace all `use` statements:
   - `use SamanSEO\` → `use SamanLabs\SEO\`
...
**Verification:** `grep -r "SamanSEO" includes/` returns no results (except comments if any)
...
1. Update autoloader namespace prefix check:
   - `strpos( $class, 'SamanSEO\\' )` → `strpos( $class, 'SamanLabs\\SEO\\' )`

2. Update all namespace-specific path mappings:
   - `SamanSEO\\Api\\` → `SamanLabs\\SEO\\Api\\`
   - `SamanSEO\\Integration\\` → `SamanLabs\\SEO\\Integration\\`
   - `SamanSEO\\Service\\` → `SamanLabs\\SEO\\Service\\`
...
2. **Constant check:** `grep -r "SAMANSEO_" includes/ templates/` - no results
3. **Namespace check:** `grep -r "SamanSEO" includes/` - no results (except docs)
