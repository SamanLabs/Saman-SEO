...
/**
 * Make a tag pill selectable with a single click.
 *
...
const createTagFromRaw = ( raw ) => {
	...
	span.className = `template-input-v2__tag template-input-v2__tag--${ varType }`;
...
const readValue = ( root ) => {
	...
	root.childNodes.forEach( ( node ) => {
		if ( node.nodeType === Node.TEXT_NODE ) {
			...
			if (
				node.classList &&
				node.classList.contains( 'template-input-v2__tag' )
			) {
...
const TemplateInput = ( {
	...
	const insertVariable = ( variableTag ) => {
		...
		tagEl.className = `template-input-v2__tag template-input-v2__tag--${ varType }`;
	...
	return (
		<div
			className={ `template-input-v2 ${ isFocused ? 'is-focused' : '' } ${
		...
		>
			...
			<div ref={ containerRef } className="template-input-v2__container">
				<div
					ref={ editorRef }
					id={ id }
					className={ `template-input-v2__field ${
						multiline ? 'multiline' : ''
					}` }
					...
					onDrop={ ( event ) => {
						...
						const dragged = Array.from(
							editor.querySelectorAll( '.template-input-v2__tag' )
				...
				/>
				{ ! value && ! isFocused && (
					<span className="template-input-v2__placeholder">
						{ placeholder }
					</span>
				) }
				{ hoveredVariable &&
			...
			{ ( helpText || maxLength ) && (
				<div className="template-input-v2__footer">
					{ helpText && (
						<span className="template-input-v2__help">
							{ helpText }
						</span>
					) }
					{ maxLength && (
						<span
							className={ `template-input-v2__counter ${
