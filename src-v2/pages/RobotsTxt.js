	...
	const validateContent = ( text ) => {
		...
		lines.forEach( ( line, index ) => {
			...
			if ( ! validDirectives.includes( directivePart ) ) {
				warnings.push(
					`Line ${ lineNum }: Unknown directive "${ directivePart }"`
				);
			}
			...
			if (
				...
				errors.push(
					`Line ${ lineNum }: ${ directivePart } must come after User-agent`
				);
			}
		...
		<div className="page">
			<div className="page-header">
				<div>
					<h1>robots.txt Editor</h1>
					<p>
						Control how search engines crawl your site. Changes are
						saved to the database and override the default WordPress
						robots.txt.
					</p>
				</div>
