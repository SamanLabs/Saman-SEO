		...
		const loadPlugins = async () => {
			try {
				...
				const data = await apiFetch( {
					path: '/saman-seo/v1/more/plugins',
				} );
				setPlugins( data || [] );
			} catch ( err ) {
						...
						>
							<div className="managed-plugin-header">
								<div className="managed-plugin-icon">
									...
									<div
										className={ `managed-plugin-icon-fallback ${
											plugin.type || 'default'
										}` }
										style={ {
											display: plugin.icon
							...
							<p className="managed-plugin-description">
								{ plugin.description }
							</p>
