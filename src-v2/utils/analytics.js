...
export const trackEvent = ( category, action, name = null, value = null ) => {
	...
	try {
		...
		if ( window.SamanSEODebug ) {
			console.log( 'Saman SEO Analytics:', {
				category,
				action,
				name,
				value,
...
export const analytics = {
	...
	redirects: {
		...
		import: ( count ) =>
			trackEvent( 'Redirects', 'Import', 'count', count ),
		export: ( count ) =>
			trackEvent( 'Redirects', 'Export', 'count', count ),
	},
	...
	ai: {
		...
		assistant: ( assistantId ) =>
			trackEvent( 'AI', 'Assistant', assistantId ),
	...
	searchAppearance: {
		...
		aiGenerate: ( field ) =>
			trackEvent( 'SearchAppearance', 'AIGenerate', field ),
	},
	...
	internalLinking: {
		...
		applySuggestion: () =>
			trackEvent( 'InternalLinking', 'ApplySuggestion' ),
	...
	tools: {
		...
		schemaBuilder: ( schemaType ) =>
			trackEvent( 'Tools', 'SchemaBuilder', schemaType ),
	},

	// Settings
	settings: {
		save: ( tab ) => trackEvent( 'Settings', 'Save', tab ),
		moduleToggle: ( module, enabled ) =>
			trackEvent( 'Settings', 'ModuleToggle', module, enabled ? 1 : 0 ),
