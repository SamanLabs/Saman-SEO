...
class Admin_Bar {
	...
	public function boot() {
		...
		add_action( 'admin_bar_menu', array( $this, 'add_seo_menu' ), 100 );
	...
	private function add_post_seo_menu( $wp_admin_bar, $post ) {
		...
		$wp_admin_bar->add_node(
			array(
				...
				'meta'   => array( 'class' => 'saman-seo-ab-score-item' ),
			)
		);
		...
		$wp_admin_bar->add_node(
			array(
				...
				'meta'   => array( 'class' => 'saman-seo-ab-status-item' ),
			)
		);
		...
		if ( $issue_count > 0 ) {
			$wp_admin_bar->add_node(
				array(
					...
					'meta'   => array( 'class' => 'saman-seo-ab-issues-item' ),
				)
			);
		...
		$wp_admin_bar->add_node(
			array(
				...
				'meta'   => array( 'class' => 'saman-seo-ab-separator-item' ),
			)
		);
		...
		$wp_admin_bar->add_node(
			array(
				...
				'meta'   => array( 'class' => 'saman-seo-ab-action-link' ),
			)
		);
		...
		foreach ( $nav_items as $key => $item ) {
			...
			$wp_admin_bar->add_node(
				array(
					...
					'href'   => $item['url'],
					'meta'   => array( 'class' => 'saman-seo-ab-nav-link' ),
				)
			);
		}
	}
	...
	private function get_seo_data( $post ) {
		...
		return array(
			...
			'issues' => isset( $score_data['issues'] ) && is_array( $score_data['issues'] ) ? $score_data['issues'] : array(),
		);
	}
