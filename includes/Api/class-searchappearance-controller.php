...
class SearchAppearance_Controller extends REST_Controller {
	 ...
	 * @var array
	 */
	private $schema_page_options = array(
	 ...
	 * @var array
	 */
	private $schema_article_options = array(
	 ...
	 * @var array
	 */
	private $schema_medical_options = array(
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => \WP_REST_Server::READABLE,
					'callback'            => array( $this, 'get_homepage_defaults' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
				array(
					'methods'             => \WP_REST_Server::CREATABLE,
					'callback'            => array( $this, 'save_homepage_defaults' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
			)
		);
		...
		register_rest_route(
			...
			array(
				...
				array(
					'methods'             => \WP_REST_Server::CREATABLE,
					'callback'            => array( $this, 'save_post_types' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
			)
		);
		...
		register_rest_route(
			$this->namespace,
			'/search-appearance/post-types/(?P<slug>[a-zA-Z0-9_-]+)',
			array(
				array(
					'methods'             => \WP_REST_Server::CREATABLE,
					'callback'            => array( $this, 'save_single_post_type' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
			)
		);
		...
		register_rest_route(
			...
			array(
				...
				array(
					'methods'             => \WP_REST_Server::CREATABLE,
					'callback'            => array( $this, 'save_taxonomies' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
			)
		);
		...
		register_rest_route(
			$this->namespace,
			'/search-appearance/taxonomies/(?P<slug>[a-zA-Z0-9_-]+)',
			array(
				array(
					'methods'             => \WP_REST_Server::CREATABLE,
					'callback'            => array( $this, 'save_single_taxonomy' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
			)
		);
		...
		register_rest_route(
			...
			array(
				...
				array(
					'methods'             => \WP_REST_Server::CREATABLE,
					'callback'            => array( $this, 'save_archives' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
			)
		);
		...
		register_rest_route(
			$this->namespace,
			'/search-appearance/social-defaults',
			array(
				array(
					'methods'             => \WP_REST_Server::READABLE,
					'callback'            => array( $this, 'get_social_defaults' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
				array(
					'methods'             => \WP_REST_Server::CREATABLE,
					'callback'            => array( $this, 'save_social_defaults' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
			)
		);
		...
		register_rest_route(
			$this->namespace,
			'/search-appearance/social-defaults/(?P<slug>[a-zA-Z0-9_-]+)',
			array(
				array(
					'methods'             => \WP_REST_Server::CREATABLE,
					'callback'            => array( $this, 'save_post_type_social' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
			)
		);
		...
		register_rest_route(
			$this->namespace,
			'/search-appearance/card-design',
			array(
				array(
					'methods'             => \WP_REST_Server::READABLE,
					'callback'            => array( $this, 'get_card_design' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
				array(
					'methods'             => \WP_REST_Server::CREATABLE,
					'callback'            => array( $this, 'save_card_design' ),
					'permission_callback' => array( $this, 'permission_check' ),
				),
			)
		);
	...
	private function get_post_types_data() {
		...
		$defaults          = get_option( 'SAMAN_SEO_post_type_defaults', array() );
		$settings          = get_option( 'SAMAN_SEO_post_type_settings', array() );
		$title_templates   = get_option( 'SAMAN_SEO_post_type_title_templates', array() );
		$meta_descriptions = get_option( 'SAMAN_SEO_post_type_meta_descriptions', array() );

		$data = array();
		foreach ( $post_types as $slug => $post_type ) {
			$pt_defaults = isset( $defaults[ $slug ] ) ? $defaults[ $slug ] : array();
			$pt_settings = isset( $settings[ $slug ] ) ? $settings[ $slug ] : array();
	...
	private function get_taxonomies_data() {
		...
		$defaults = get_option( 'SAMAN_SEO_taxonomy_defaults', array() );
		$settings = get_option( 'SAMAN_SEO_taxonomy_settings', array() );

		$data = array();
		foreach ( $taxonomies as $slug => $taxonomy ) {
			$tax_defaults = isset( $defaults[ $slug ] ) ? $defaults[ $slug ] : array();
			$tax_settings = isset( $settings[ $slug ] ) ? $settings[ $slug ] : array();
	...
	private function get_archives_data() {
		$defaults = get_option( 'SAMAN_SEO_archive_defaults', array() );
		$settings = get_option( 'SAMAN_SEO_archive_settings', array() );
		...
		$archive_types = array(
			'author' => array(
				...
				'variables'                    => array( 'author', 'author_bio', 'separator', 'site_title' ),
				'sample_values'                => array(
			...
			'date'   => array(
				...
				'variables'                    => array( 'date', 'separator', 'site_title' ),
				'sample_values'                => array(
					'date' => date_i18n( 'F Y' ),
				),
			),
			'search' => array(
				...
				'variables'                    => array( 'search_term', 'separator', 'site_title' ),
				'sample_values'                => array(
			...
			'404'    => array(
				...
				'variables'                    => array( 'request_url', 'separator', 'site_title' ),
				'sample_values'                => array(
		...
		foreach ( $archive_types as $slug => $info ) {
			$archive_defaults = isset( $defaults[ $slug ] ) ? $defaults[ $slug ] : array();
			$archive_settings = isset( $settings[ $slug ] ) ? $settings[ $slug ] : array();
			...
			} elseif ( in_array( $slug, array( 'search', '404' ), true ) ) {
	...
	private function get_social_defaults_data() {
		$defaults = get_option( 'SAMAN_SEO_social_defaults', array() );
	...
	private function get_post_type_social_defaults_data() {
		$defaults = get_option( 'SAMAN_SEO_post_type_social_defaults', array() );
	...
	public function save_post_type_social( $request ) {
		...
		$all_defaults = get_option( 'SAMAN_SEO_post_type_social_defaults', array() );

		if ( ! is_array( $all_defaults ) ) {
			$all_defaults = array();
		}

		$all_defaults[ $slug ] = array(
			'og_title'            => isset( $params['og_title'] ) ? sanitize_text_field( $params['og_title'] ) : '',
			'og_description'      => isset( $params['og_description'] ) ? sanitize_textarea_field( $params['og_description'] ) : '',
			'twitter_title'       => isset( $params['twitter_title'] ) ? sanitize_text_field( $params['twitter_title'] ) : '',
			'twitter_description' => isset( $params['twitter_description'] ) ? sanitize_textarea_field( $params['twitter_description'] ) : '',
	...
	private function get_card_design_data() {
		$design = get_option( 'SAMAN_SEO_card_design', array() );
	...
	public function save_card_design( $request ) {
		...
		// Validate layout
		$allowed_layouts = array( 'default', 'centered', 'minimal', 'magazine', 'gradient', 'corner' );
