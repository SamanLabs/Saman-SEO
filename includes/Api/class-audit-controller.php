...
class Audit_Controller extends REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => \WP_REST_Server::READABLE,
					'callback'            => array( $this, 'get_post_issues' ),
					'permission_callback' => array( $this, 'permission_check' ),
					'args'                => array(
						'id' => array(
							'type'              => 'integer',
	...
	public function get_post_issues( $request ) {
		...
		$issues = $this->extract_issues_from_metrics( $score_data['metrics'] ?? array() );
		...
		return $this->success(
			array(
				...
				'metrics_by_category' => $this->group_metrics_by_category( $score_data['metrics'] ?? array() ),
				'issues'              => $issues,
	...
	private function group_metrics_by_category( $metrics ) {
		$groups = array(
			...
			'keyword'   => array(
				'label' => __( 'Keyword Optimization', 'saman-seo' ),
				'items' => array(),
			),
			'structure' => array(
